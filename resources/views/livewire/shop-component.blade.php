<div class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white font-sans antialiased selection:bg-cyan-500 selection:text-slate-950">
    
    <!-- 🌟 تأثيرات خلفية -->
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
        <div class="absolute -top-40 -left-40 w-96 h-96 bg-cyan-500/10 rounded-full blur-3xl animate-pulse"></div>
        <div class="absolute top-60 -right-40 w-80 h-80 bg-blue-600/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10">
            ...
            <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="relative">
                        ...
                        <div class="w-3 h-3 rounded-full bg-cyan-400 relative ring-2 ring-cyan-400/30"></div>
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-3 w-full lg:w-auto justify-between lg:justify-end">
                    <span class="text-xs text-slate-400">👋 مرحباً: <strong class="text-cyan-400 font-bold bg-cyan-400/10 px-2 py-0.5 rounded-lg">{{ auth()->user()->name ?? 'زائرنا الكريم' }}</strong></span>
                    @auth
                        <a href="/admin" class="group relative px-4 py-2 text-xs font-bold rounded-xl overflow-hidden transition-all duration-300 hover:scale-105">
                            <span class="absolute inset-0 bg-slate-800/80 border border-slate-700 rounded-xl group-hover:border-cyan-500/50 transition-colors"></span>
                            <span class="relative flex items-center gap-2 text-slate-300 group-hover:text-cyan-400 transition-colors">⚙️ لوحة التحكم</span>
                        </a>
                    @else
                        <a href="/login" class="relative px-6 py-2.5 text-xs font-bold rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 transition-all duration-300 hover:scale-105">🔐 تسجيل الدخول</a>
                    @endauth
                </div>
            </div>
        ...
        @if (session()->has('success'))
            <div class="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500/15 to-emerald-500/5 border border-emerald-500/20 backdrop-blur-sm shadow-xl shadow-emerald-500/5 animate-slideDown">
                ...
                <div class="relative p-5 flex items-center gap-4">
                    <div class="flex-shrink-0 w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center backdrop-blur-sm">
                        <svg class="w-6 h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    </div>
                    <div>
                        <p class="font-bold text-emerald-400 text-sm">✅ تم بنجاح!</p>
        ...
        @if (session()->has('error'))
            <div class="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-red-500/15 to-red-500/5 border border-red-500/20 backdrop-blur-sm shadow-xl shadow-red-500/5 animate-slideDown">
                <div class="absolute inset-0 bg-gradient-to-r from-red-500/5 to-transparent"></div>
                <div class="relative p-5 flex items-center gap-4">
                    <div class="flex-shrink-0 w-12 h-12 rounded-full bg-red-500/20 flex items-center justify-center backdrop-blur-sm">
                        <svg class="w-6 h-6 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    </div>
                    <div>
                        <p class="font-bold text-red-400 text-sm">⚠️ تنبيه!</p>
        ...
        <div class="mb-8">
            <div class="flex items-center gap-2 overflow-x-auto pb-3">
                ...
                @foreach($categories as $category)
                    <button wire:click="filterCategory({{ $category->id }})" class="px-5 py-2.5 rounded-2xl text-xs font-bold transition-all duration-300 shrink-0 whitespace-nowrap {{ $selectedCategory == $category->id ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-slate-950 shadow-lg shadow-cyan-500/30 scale-105' : 'bg-slate-800/50 border border-slate-700/50 text-slate-400 hover:text-white hover:border-slate-600 hover:scale-105' }}">
                        {{ $category->name }}
                    </button>
                @endforeach
            </div>
                        ...
                        <div class="group relative bg-gradient-to-br from-slate-900/80 to-slate-900/40 backdrop-blur-sm border border-slate-800/50 rounded-2xl p-5 hover:border-cyan-500/40 transition-all duration-500 hover:shadow-2xl hover:shadow-cyan-500/10 hover:-translate-y-2">
                            ...
                            <div class="mt-5 flex items-center justify-between pt-4 border-t border-slate-800/50">
                                <div>
                                    ...
                                    <div class="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-500">{{ number_format($product->price) }} <span class="text-xs font-medium text-slate-400">ج.م</span></div>
                                </div>
                                @if($product->stock > 0)
                                    <button wire:click="addToCart({{ $product->id }})" class="relative px-5 py-2.5 rounded-xl text-xs font-black text-slate-950 overflow-hidden group/btn transition-all duration-300 hover:scale-105 active:scale-95">
                                        ...
                                        <span class="relative z-10 flex items-center gap-2">🛒 إضافة</span>
                                    </button>
                                @else
                                    <span class="text-xs text-red-400 font-bold bg-red-500/10 border border-red-500/20 px-4 py-2.5 rounded-xl">❌ نفد</span>
                                @endif
                            </div>
                        </div>
                    @empty
                        <div class="col-span-2 flex flex-col items-center justify-center py-20 text-center">
                            <div class="w-28 h-28 rounded-full bg-slate-800/50 flex items-center justify-center mb-6">
                                <svg class="w-14 h-14 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                            </div>
                            <p class="text-slate-400 text-base font-bold">لا توجد قطع متاحة</p>
                            <p class="text-slate-600 text-sm mt-1">جرب تغيير التصنيف المحدد</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="xl:col-span-4">
                <div class="sticky top-8 bg-gradient-to-br from-slate-900/80 to-slate-900/40 backdrop-blur-xl border border-slate-800/50 rounded-2xl p-6 shadow-2xl shadow-cyan-500/5">
                    <div class="flex items-center justify-between pb-4 mb-5 border-b border-slate-800/50">
                        ...
                        <span class="text-[10px] font-bold px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">{{ count($cart) }} عنصر</span>
                    </div>
                    ...
                    @if(empty($cart))
                        <div class="flex flex-col items-center py-16 text-center">
                            ...
                            <p class="text-slate-400 font-medium">🛒 السلة فارغة</p>
                            <p class="text-slate-600 text-sm mt-1">ابدأ بإضافة المنتجات</p>
                        </div>
                    @else
                        <div class="space-y-3 max-h-80 overflow-y-auto pr-1 custom-scrollbar">
                            @foreach($cart as $item)
                                <div class="group relative bg-slate-800/40 border border-slate-700/40 rounded-xl p-3 hover:border-slate-600/60 transition-all duration-300">
                                    <div class="flex items-center justify-between gap-3">
                                        <div class="flex-1 min-w-0">
                                            <h4 class="text-sm font-bold text-white truncate">{{ $item['name'] }}</h4>
                                            <span class="text-xs font-bold text-amber-400">{{ number_format($item['price']) }} ج.م</span>
                                        </div>
                                        <div class="flex items-center gap-1 bg-slate-950/80 border border-slate-700/50 rounded-lg px-1.5 py-0.5">
                                            <button wire:click="decrementQuantity({{ $item['id'] }})" class="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors duration-200 font-bold text-sm">−</button>
                                            <span class="w-6 text-center text-xs font-black text-cyan-400">{{ $item['quantity'] }}</span>
                                            <button wire:click="incrementQuantity({{ $item['id'] }})" class="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 transition-colors duration-200 font-bold text-sm">+</button>
                                        </div>
                                        <button wire:click="removeFromCart({{ $item['id'] }})" class="flex-shrink-0 p-1.5 rounded-lg text-slate-600 hover:text-red-400 hover:bg-red-500/10 transition-all duration-300">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="mt-5 pt-4 border-t border-slate-800/50 space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-500">{{ number_format($totalPrice) }} ج.م</span>
                            </div>
